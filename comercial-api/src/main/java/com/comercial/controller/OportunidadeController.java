...
public class OportunidadeController {
	...
	@PutMapping
	public void atualizar(@Valid @RequestBody Oportunidade oportunidade ) {
		Optional<Oportunidade> oportunidadeExistente = oportunidades.findById(oportunidade.getId());
		
		if(oportunidadeExistente.isPresent()) {
			throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Oportunidade não existente, não pode ser atualizada");
		}	
	...
	public void remover(@PathVariable Long id) {
		...
		if(oportunidadeExistente.isPresent()) {
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Oportunidade não existente");
		}
